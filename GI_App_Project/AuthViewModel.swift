...
final class AuthViewModel: ObservableObject {
    ...
    func registerUser(labNumber: String, item: PhotosPickerItem, nickName: String, withEmail email: String, password: String, fullName: String) async throws {
        let result = try await Auth.auth().createUser(withEmail: email, password: password)
        let profileImageURL = try await saveProfileImage(item: item)
        let user = User(labNumber: labNumber, profileImageURL: profileImageURL, nickName: nickName, fullName: fullName, email: email, uid: result.user.uid)
